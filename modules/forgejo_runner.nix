{
  ...
  # systemd CPUQuota for the runner cgroups (empty string disables). Bounds the
  # aggregate CPU of ALL CI jobs per runner unit (rustc fans out to every core,
  # and with runner_capacity > 1 several builds overlap). Motivating incident
  ...
  cpu_quota ? "",
  # systemd IOWeight for the runner cgroups (empty string disables). Unlike
  ...
  io_weight ? "",
  # One entry per runner instance. Each registers with its own token: a token
  # issued from a repo's settings page scopes that runner to the repo, an org
  # token serves the whole org. `name` is the instance name (systemd unit
  # gitea-runner-<name>); `runner_name` is what Forgejo's UI shows (defaults to
  # the host name).
  runners ? [
    {
      name = "default";
...
in {
  ...
  # names must use the same escaping the gitea-actions-runner module applies,
  # or the override lands in a phantom unit and never reaches the service.
  ...
  systemd.services =
    lib.listToAttrs (
      map (runner: {
        name = "gitea-runner-${utils.escapeSystemdPath runner.name}";
        value.serviceConfig =
          {
    ...
    // {
      ...
      nexus-ci-cache-prune = lib.mkIf (ci_cache_dir != "") {
